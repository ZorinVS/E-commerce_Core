...
- Order: Класс для представления заказа.
  ...
  - Методы:
    - `__init__`: Конструктор для создания экземпляра заказа, проверяет наличие достаточного количества товара на складе. Уменьшает количество товара на складе на величину, соответствующую количеству в заказе. Если товара недостаточно, выбрасывает ValueError.
